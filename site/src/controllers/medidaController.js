...
function buscarMedidasExportar(req, res) {
    var dataInicio = req.params.dataInicio;
    var dataFim = req.params.dataFim;

    medidaModel.buscarMedidasExportar(dataInicio, dataFim).then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
            res.status(204).send("Nenhum resultado encontrado!")
